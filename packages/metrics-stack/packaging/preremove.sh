...
# unguarded %preun would stop every service right after the new package's
# %post restarted them, leaving the stack down after `dnf upgrade`. rpm
# passes $1 as the count of instances remaining (0 = full erase, >=1 =
# upgrade); dpkg passes a word ("remove", "upgrade", ...). On deb the
# ordering is prerm(old) then postinst(new), so skipping the upgrade case
# is harmless there too -- postinst restarts everything anyway.
case "${1:-0}" in
  0|remove) ;;
  *) exit 0 ;;
esac
